...
def getInput(n , m ,P_board):
     # get individual message input from user
    for x in range(n):
        for y in range(m):
                P_board[x][y].message = input("Enter an event Value: ")

def PrintBoard(n,m,P_board):
      # printing the Process Board
    for x in range(n):
        print('\n')
        for y in range(m):
            print(P_board[x][y].message, end=" ")

def PrintTime(n,m,P_board):
# outputting time values of messages
    for x in range(n):
        print('\n')
        for y in range(m):
            print(P_board[x][y].get_Time(), end=" ")
...
def main():
    ...
    getInput(n, m,P_board1)

    print("\n================Initial Process Board===============")
  
    PrintBoard(n,m,P_board1)
